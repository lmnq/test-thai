...
func (r *GroupRepo) GetIDByName(ctx context.Context, name string) (int, error) {
	var groupID int
	q := `SELECT id FROM tbl_groups WHERE group_name = $1 AND deleted_at IS NULL`
	err := r.Pool.QueryRow(ctx, q, name).Scan(&groupID)
	if err == pgx.ErrNoRows {
